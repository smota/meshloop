...
| `ctx.parse.latency_ms.p95` | <= 15.0ms | 0.015 ms | **PASS** |
| `ctx.cache.stale_hit_rate_pct` | = 0.0% | 0.0% | **PASS** |
| `quant.index.compression_ratio` | >= 4.0x | 5.5x | **PASS** |
| `quant.search.latency_us.p95` | <= 500.0us | 484.0 us | **PASS** |
| `quant.recall_at_k` | >= 95.0% | 100.0% | **PASS** |
| `orch.txn.wal_commit_ms.p95` | <= 10.0ms | 1.09 ms | **PASS** |
| `conc.wal.write_contention_ms` | <= 15.0ms | 1.09 ms | **PASS** |
| `conc.git_admin.lock_contention_ms` | <= 200.0ms | 64.25 ms | **PASS** |
| `conc.throughput_gain` | >= 1.0x | 2.55x | **PASS** |
