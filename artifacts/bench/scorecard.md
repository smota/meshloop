...
| `quant.search.latency_us.p95` | <= 500.0us | 313.0 us | **PASS** |
| `quant.recall_at_k` | >= 95.0% | 100.0% | **PASS** |
| `orch.txn.wal_commit_ms.p95` | <= 10.0ms | 1.06 ms | **PASS** |
| `conc.wal.write_contention_ms` | <= 15.0ms | 0.99 ms | **PASS** |
| `conc.git_admin.lock_contention_ms` | <= 200.0ms | 73.16 ms | **PASS** |
| `conc.throughput_gain` | >= 1.0x | 1.85x | **PASS** |
...
| `orch.schedule.overhead_ms.p95` | <= 25.0ms | 0.054 ms (7 manifests) | **PASS** |
| `conc.orphan_process_count` | = 0.0 | 0 | **PASS** |
| `iso.ancestor_propagation.pass_rate_pct` | = 100.0% | 100.0% | **PASS** |
| `orch.wave.dispatch_overhead_ms` | <= 1500.0ms | 965.1 ms | **PASS** |
| `iso.mutation_rollback.fidelity` | = 1.0 | 1.0 (7/7 negative controls rolled back) | **PASS** |
| `orch.mutation.overhead_ms.p95` | <= 15.0ms | 1.745 ms (6 manifests) | **PASS** |
| `smoke.wall_clock_s` | <= 5.0s | 4.01 s | **PASS** |
...
**Overall Gate Status:** PASS (23/23 metrics within canonical thresholds)
